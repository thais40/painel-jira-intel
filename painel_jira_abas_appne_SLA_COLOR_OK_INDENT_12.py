...
# ========== INTERFACE POR PROJETO ==========
    for i, projeto in enumerate(PROJETOS):
        with abas[i]:
            ...
            sla_mes["mes_str"] = pd.Categorical(sla_mes["mes_str"], categories=sorted(sla_mes["mes_str"].unique(), key=lambda x: pd.to_datetime(x, format="%b/%Y")))
            ...
            fig_sla.update_layout(barmode="stack", title="SLA Dentro | Fora", xaxis_title="Mês", yaxis_title="Chamados")
            ...
            st.plotly_chart(fig, use_container_width=True, key=f"barras_{projeto}")
pass  # 🙌 raiz limpa, não tem erro
with st.container():
