...
PAINEL_PASSWORD = st.secrets.get("PAINEL_PASSWORD", "painel123")
...
with st.expander("ℹ️ Sobre o Painel"):
    st.markdown("""
    Este painel coleta dados diretamente da API do Jira e apresenta indicadores por projeto:
    - TDS (inclui App NE/EN)
    - INT
    - TI NE
    - INTEL
    """)
    ...
    for projeto in PROJETOS:
        ...
        while True:
            ...
            if res.status_code != 200:
                st.error(f"Erro ao acessar Jira para {projeto}: {res.status_code}")
                break
...
if df.empty:
    ...
    st.metric("Tickets dentro do SLA", int(df["dentro_sla"].sum()))
