...
fig = px.bar(grafico, x='mes_str', y=['Criados', 'Resolvidos'], barmode='group', title='Tickets Criados | Resolvidos', text='value')
        ...
        categories=sorted(sla_mes['mes_str'].unique(), key=lambda x: pd.to_datetime(x, format='%b/%Y'))
            ...
            fig_sla = go.Figure()
            fig_sla.add_trace(go.Bar(x=sla_mes['mes_str'], y=sla_mes['sum'], name='Dentro SLA', marker_color='green', text=sla_mes['sum'], textposition='outside')
, text=sla_mes['sum'], textposition='outside')

            fig_sla.add_trace(go.Bar(x=sla_mes['mes_str'], y=sla_mes['fora'], name='Fora SLA', marker_color='red', text=sla_mes['fora'], textposition='outside')
, text=sla_mes['fora'], textposition='outside')
        ...
        st.plotly_chart(fig_sla, use_container_width=True)
            ...
            # TABELA DE ÁREA SOLICITANTE POR MÊS
        ...
        x='Chamados',
        orientation='h',
        title='Tickets por Área Solicitante',
        labels={'area': 'Área Solicitante', 'Chamados': 'Qtd. Chamados'}
            )
        st.plotly_chart(fig_area, use_container_width=True)

            # GRÁFICO ASSUNTO RELACIONADO
            assuntos_expandidos = dfx.explode('assunto')
            assunto_contagem = assuntos_expandidos['assunto'].value_counts().reset_index()
            assunto_contagem.columns = ['Assunto', 'Qtd. Chamados']
            fig_assunto = px.bar(
        assunto_contagem,
        y='Assunto',
        x='Qtd. Chamados',
        orientation='h',
        text='Qtd. Chamados',
        title='Tickets - Assunto relacionado'
            )
