...
def processar(df):
    ...
    df["mes_resolucao"] = df["resolved"].dt.to_period("M").dt.to_timestamp()
    return df

# --- UI ---
st.set_page_config(layout="wide")
st.title("📊 Painel de Indicadores - Projeto INT (últimos 6 meses, ordenado)")
...
if df.empty:
    ...
    grafico["mes_str"] = grafico["mes"].dt.strftime("%b/%Y")
    fig = go.Figure([
    ...
    sla_percent["mes_str"] = sla_percent["mes"].dt.strftime("%b/%Y")
    fig_sla = go.Figure([
        go.Bar(
            name="% dentro SLA",
            x=sla_percent["mes_str"],
    ...
    area_mes = df.groupby(["mes", "area"]).size().reset_index(name="Chamados")
    area_mes["mes_str"] = area_mes["mes"].dt.strftime("%b/%Y")
    st.dataframe(area_mes.pivot(index="mes_str", columns="area", values="Chamados").fillna(0).astype(int))
