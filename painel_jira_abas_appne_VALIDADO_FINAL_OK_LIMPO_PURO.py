    ...
    for projeto in PROJETOS:
        ...
        while True:
            ...
            for issue in issues:
                ...
                escalado_n3 = f.get("customfield_13659", {}).get("value") == "Sim"
                all_issues.append({
                    "key": issue["key"],
                    "projeto": projeto,
                    "created": f["created"],
...
# ========== INTERFACE POR PROJETO ==========
    for i, projeto in enumerate(PROJETOS):
        with abas[i]:
            ...
            mes_gauge = st.selectbox("🗓️ Selecione o mês para o indicador de SLA:", options=sla_mes["mes_str"].tolist(), index=len(sla_mes)-1, key=f"mes_gauge_{projeto}")
            ...
            fig_sla.update_layout(title="% de Chamados dentro do SLA", yaxis_title="%", xaxis_title="Mês",
                ...
                text=f"🎯 Meta SLA {sla_meta:.0f}%", showarrow=False,
                ...
                with col2:
                    fig_gauge = go.Figure(go.Indicator(
                        ...
                        gauge={
                            ...
                            "bar": {"color": "darkgreen"},
                            "steps": [
                                {"range": [0, sla_meta], "color": "red"},
                                {"range": [sla_meta, 100], "color": "green"}
                            ],
                            "threshold": {
