...
SLA_ALVO_HORAS = 40
SLA_ALVO_DIAS = SLA_ALVO_HORAS / 8  # considerando 8h por dia útil
...
if df.empty:
    ...
    resolvidos = df.dropna(subset=["resolved"]).groupby(df["resolved"].dt.to_period("M").astype(str)).size().reset_index(name="Resolvidos")
    grafico = pd.merge(criados, resolvidos, how="outer", on="mes").fillna(0).sort_values("mes")
    st.bar_chart(grafico.set_index("mes"))

    st.markdown("### ⏱️ SLA por mês com meta (96%)")
    sla_group = df.groupby("mes")["dentro_sla"].agg(["sum", "count"]).reset_index()
    sla_group["% dentro"] = (sla_group["sum"] / sla_group["count"]) * 100
